 ...
 *  </FileTree>
 *
 *  Please note that `"categories"` is not supported when the [`categorizeByGroup`](https://typedoc.org/documents/Options.Organization.html#categorizebygroup) TypeDoc organization option is set to `true`.
 *
 * **"modules"**
 *
  ...
  validate(values) {
    ...
    for (const kind of values) {
      if (!validValues.includes(kind)) {
        throw new Error(
          `'[typedoc-plugin-markdown] ${kind}' is an invalid value for 'membersWithOwnFile'. Must be one of: ${validValues.join(
            ', ',
          )}`,
        );
      }
